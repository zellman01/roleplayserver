...
if (RPC.ExpControl) {
	RPC.ExpControl.end();
	delete RPC.ExpControl;
}
RPC.ExpControl = new ExpFunctions();
...
exports.commands = {
	...
	exp: function (target, room, user) {
		...
		if (target || !target && this.broadcasting) {
			...
			EXP.readExp(targetId, exp => {
				this.sendReplyBox(`${RPC.nameColor(targetId, true)} has ${exp} exp and is level ${RPC.ExpControl.level(targetId)} and needs ${RPC.ExpControl.nextLevel(targetId)} to reach the next level.`);
			});
		} else {
			EXP.readExp(user.userid, exp => {
				this.sendReplyBox(
					"Name: " + RPC.nameColor(user.userid, true) + "<br />Current level: " + RPC.ExpControl.level(user.userid) + "<br />Current Exp: " + exp + "<br />Exp Needed for Next level: " + RPC.ExpControl.nextLevel(user.userid) +
					"<br />Rewards<br /><br />" +
					"Level 5 unlocks requesting legends"
				);
			});
		}
	},
	...
	giveexp: function (target, room, user) {
		...
		RPC.ExpControl.addExp(uid, room, amount);
	...
	resetxp: function (target, room, user, conection, cmd) {
		...
		if (cmd !== "confirmresetexp") {
			return this.popupReply(`|html|<center><button name="send" value="/confirmresetexp ${targetUser}" style="background-color: red; height: 300px; width: 150px"><strong><font color= "white" size= 3>Confirm XP reset of ${RPC.nameColor(targetUser, true)}; this is only to be used in emergencies, cannot be undone!</font></strong></button>`);
		}
		...
		user.popup(`|html|You have reset the XP of ${RPC.nameColor(targetUser, true)}.`);
